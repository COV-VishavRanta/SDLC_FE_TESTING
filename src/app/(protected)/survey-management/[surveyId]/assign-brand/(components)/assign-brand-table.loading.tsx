...
function AssignBrandTableSkeletonRows({ count }: { count: number }) {
...
export function AssignBrandTableSkeleton() {
  return (
    <div
      aria-busy='true'
      aria-label='Loading assign brand page'
 ...
 */
export function AssignBrandTableInlineLoading({ rowCount }: { rowCount: number }) {
  const count = Math.max(rowCount, 1);
  return <AssignBrandTableSkeletonRows count={count} />;
}
