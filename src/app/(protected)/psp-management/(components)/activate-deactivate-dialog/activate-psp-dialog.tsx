...
interface ActivatePspDialogProps {
  psp: PSPType;
  onClose: () => void;
  handleActivatePsp: (id: string) => Promise<void>;
  isUpdatingStatus: boolean;
}

export function ActivatePspDialog({
  psp,
  onClose,
  handleActivatePsp,
  isUpdatingStatus,
}: ActivatePspDialogProps) {
  const t = useTranslations('pspManagement.activateDialog');

  const handleActivate = async () => {
    await handleActivatePsp(psp.id);
    onClose();
  };
      ...
      >
        ...
        <DialogHeader className='gap-2 px-4 py-4 sm:px-8 sm:py-6'>
          <div className='flex items-center justify-between'>
            <DialogTitle className='text-[18px] sm:text-[20px] font-semibold leading-[30px] text-text-heading'>
              {t('title')}
            </DialogTitle>
          </div>
