...
import { DELETE_INVENTORY, DeleteInventoryResponse, DeleteInventoryVariables } from '@/graphql';
...
export function DeleteInventoryDialog({
  ...
  const t = useTranslations('inventoryManagement.deleteDialog');
  ...
  const handleDelete = async () => {
    await deleteInventory({
      ...
      onCompleted: ({ deleteInventory: result }) => {
        if (result.success) {
          toast.success(t('successMessage', { name: inventoryName }));
          onOpenChange(false);
        } else {
    ...
    <Dialog open={open} onOpenChange={loading ? undefined : onOpenChange}>
      ...
      >
          ...
          <DialogClose
            render={
              <button
                type='button'
                disabled={loading}
                className='flex size-9 shrink-0 cursor-pointer items-center justify-center rounded-md text-text-secondary transition-colors hover:bg-input-bg hover:text-text-heading disabled:pointer-events-none disabled:opacity-50'
              >
        ...
        <DialogFooter className='flex flex-row items-center justify-end gap-3 border-t border-[#e5e7eb] px-6 py-6'>
          ...
          <Button
            isLoading={loading}
            className='h-[45px] rounded-lg bg-gradient-to-b from-[#A80000] to-[#dc2626] px-6 font-semibold text-white hover:from-[#e04848] hover:to-[#c42222] sm:text-[16px]'
