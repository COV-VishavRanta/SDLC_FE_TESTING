...
interface ActivateStoreDialogProps {
  store: StoreType;
  onClose: () => void;
  handleActivateStore: (id: string) => Promise<void>;
  isUpdatingStatus: boolean;
}

export function ActivateStoreDialog({
  store,
  onClose,
  handleActivateStore,
  isUpdatingStatus,
}: ActivateStoreDialogProps) {
  const t = useTranslations('storeManagement.activateDialog');

  const handleActivate = async () => {
    await handleActivateStore(store.id);
    onClose();
  };
