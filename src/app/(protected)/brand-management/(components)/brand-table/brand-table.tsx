...
import { getBrandColumns } from './brand-table-columns';
import { BrandTableInlineLoading } from './brand-table.loading';
...
export function BrandTable() {
  ...
  const brandColumns = getBrandColumns(t);
  const { brandList, paginationInfo, loading, sorting, setSorting, pagination, setPagination } =
    useContext(BrandManagementContext);

  // eslint-disable-next-line react-hooks/incompatible-library
  const table = useReactTable({
    // Apollo returns DeepPartialObject<T>; we assert the full type since all
    // required fields are guaranteed by the GraphQL response shape.
    data: brandList,
    ...
    <Card className='overflow-clip rounded-xl border border-border p-0'>
      ...
      <Table aria-busy={loading}>
        ...
        <DataTableBody
          ...
          loadingContent={<BrandTableInlineLoading rowCount={rowsOnPage} />}
        />
      </Table>
