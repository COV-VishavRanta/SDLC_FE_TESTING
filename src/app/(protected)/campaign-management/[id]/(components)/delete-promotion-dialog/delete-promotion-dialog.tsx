...
import { DELETE_PROMOTION, DeletePromotionResponse, DeletePromotionVariables } from '@/graphql';
...
export function DeletePromotionDialog({
  ...
  const t = useTranslations('campaignManagement.details.deletePromotion');

  const [deletePromotion, { loading }] = useMutation<
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
            ...
            <div className='flex items-start gap-4'>
              ...
              <div className='flex flex-col gap-1'>
                ...
                <p className='text-[14px] font-normal leading-[21px] tracking-[-0.15px] text-text-secondary'>
                  {t('confirmDescription', { name: promotionName })}
                </p>
              </div>
            </div>
          }
        ...
        <DialogFooter className='flex flex-row items-center justify-end gap-3 border-t border-[#e5e7eb] px-6 py-6'>
          <Button
            variant='outline'
            className='h-[47px] flex-1 cursor-pointer rounded-lg border-[var(--neutral-300)] bg-[var(--neutral-200)] px-6 font-medium text-[var(--neutral-900)] hover:bg-[var(--neutral-300)] sm:text-[16px]'
          ...
          <Button
            className='h-[45px] flex-1 cursor-pointer rounded-lg bg-gradient-to-b from-[#A80000] to-[#dc2626] px-6 font-semibold text-white hover:from-[#e04848] hover:to-[#c42222] sm:text-[16px]'
